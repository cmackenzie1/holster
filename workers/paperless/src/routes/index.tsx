...
const filterMiddleware = createMiddleware({ type: "function" })
  .inputValidator((data: { filterTagIds?: string[]; cursor?: string; search?: string }) => data)
  .server(({ next }) => next());
...
const getAllCorrespondents = createServerFn({ method: "GET" }).handler(async () => {
  const startTime = Date.now();
  const wideEvent: Record<string, unknown> = {
...
const getStorageStatsServer = createServerFn({ method: "GET" }).handler(async () => {
  const startTime = Date.now();
  const wideEvent: Record<string, unknown> = {
...
export const Route = createFileRoute("/")({
  ...
  loader: async ({ search }) => {
    ...
    const [documentsResult, allTags, allCorrespondents, storageStats] = await Promise.all([
      getDocuments({ data: { search: searchQuery || undefined } }),
      getAllTags(),
      getAllCorrespondents(),
      getStorageStatsServer(),
    ]);
    return { documentsResult, allTags, allCorrespondents, storageStats, initialSearch: searchQuery };
  },
});
              ...
              <tbody className="bg-slate-800">
                {table.getRowModel().rows.map((row) => (
                  <tr
                    key={row.id}
                    onClick={() =>
                      navigate({ to: "/documents/$id", params: { id: row.original.id } })
                    }
                    ...
                    {editingId === tag.id ? (
                      ...
                      <>
                        <div
                          ...
                          style={{
                            backgroundColor: tag.color ? `${tag.color}40` : "#374151",
                            borderColor: tag.color ?? "#4B5563",
                          }}
                        />
