import { createFileRoute, Link, useRouter, useNavigate, ErrorComponentProps } from "@tanstack/react-router";
...
function DocumentView() {
  ...
  const handleSaveASN = async () => {
    ...
    const newASN = trimmed === "" ? null : parseInt(trimmed, 10);
...
function TagSelectorModal({
  ...
  const filteredTags = allTags.filter((tag) => fuzzyMatch(tag.name, searchQuery));
            ...
            {showCreateOption && (
              ...
              >
                ...
                <span className="text-cyan-400 flex-1 text-left">
                  Create tag "<span className="font-medium">{normalizedSearch}</span>"
                </span>
              </button>
            )}
...
function CorrespondentSelectorModal({
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const response = await fetch(`/api/documents/${documentId}/correspondent`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
